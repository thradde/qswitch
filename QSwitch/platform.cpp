...
void SetForegroundWindowInternal_2(HWND hWnd)
{
...
void SetForegroundWindowInternal(HWND hWnd)
{
	...
	if ((::GetAsyncKeyState(VK_MENU) & 0x8000) != 0)	// if (bPressed)
	{
		Sleep(50);
		::keybd_event(VK_MENU, 0, KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP, 0);
	}

	if (GetForegroundWindow() != hWnd || IsIconic(hWnd) || !IsWindowVisible(hWnd))
		SetForegroundWindowInternal_2(hWnd);
}
...
//																GetAllExplorerPaths()
//
// Enumerates all windows of Windows Explorer and retrieves each viewed path.
// (see http://blogs.msdn.com/b/oldnewthing/archive/2004/07/20/188696.aspx)
// --------------------------------------------------------------------------------------------------------------------------------------------
void GetAllExplorerPaths(TMapExplorerPaths &paths)
{
	TCHAR szPath[4096];
	IShellWindows *psw;
									...
									if (SUCCEEDED(pfv->GetFolder(IID_IPersistFolder2, (void**)&ppf2)))
									{
										...
										if (SUCCEEDED(ppf2->GetCurFolder(&pidlFolder)))
										{
											if (SHGetPathFromIDList(pidlFolder, szPath))
												paths.insert(pair<HWND, RString>(hwndWBA, RString(szPath)));
